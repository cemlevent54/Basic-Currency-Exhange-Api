...
class Imdbfunctions():
    def __init__(self):
        ...
        # you can get api key from imdb-api.com . You can use it 100 times for free.
...
while True:
    ...
    if secim == "x":
        ...
        if secim == "1":
            ...
            for i in range(47):
                print("{0}-{1}-{2}".format(result["items"][i]["fullTitle"],result["items"][i]["imDbRating"],result["items"][i]["releaseState"]))
            break

        elif secim == "6":
            result = imdb.getComingSoon()
            for i in range(188):
                print("{0}-{1}".format(result["items"][i]["fullTitle"],result["items"][i]["releaseState"]))
            break
        
        elif secim == "7":
            result = imdb.getBoxOffice()
            for i in range(1,10):
                print("{0}.{1}-{2}-{3}-{4}".format(i,result["items"][i]["title"],result["items"][i]["weekend"],result["items"][i]["gross"],result["items"][i]["weeks"]))
            break
            ...
            for i in range(length):
                print("+{0}:{1}".format(result["results"][i]["title"],result["results"][i]["description"]))
            break
